...
Write-Host "   版本: 2.0" -ForegroundColor Gray
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host ""
...
function Remove-DirectorySafe {
    ...
    # 终止该目录下正在运行的进程
    $dirProcesses = Get-Process | Where-Object { $_.Path -and $_.Path.StartsWith($Path, [StringComparison]::OrdinalIgnoreCase) }
...
foreach ($varName in $envVarsToRemove) {
    ...
    if ($currentValue) {
        ...
        Write-Detail "  已删除: `$env:$varName (值为: $currentValue)"
        $cleanedEnvCount++
    }
}
...
foreach ($pattern in $logPatterns) {
    Get-ChildItem -Path $desktopPath -Filter $pattern -ErrorAction SilentlyContinue | ForEach-Object {
        if (Remove-FileSafe -Path $_.FullName -Label $_.Name) {
            $cleanedLogs++
        }
    }
}
...
# TEMP 目录
$tempFiles = @(
    (Join-Path $env:TEMP "claude-code-env.ps1"),
    (Join-Path $env:TEMP "claude-install-*.txt")
)
foreach ($tf in $tempFiles) {
    if (Test-Path $tf) {
        Remove-FileSafe -Path $tf -Label $tf | Out-Null
    }
}
...
if (Test-Path $projectDir) {
    Write-Warn "桌面 my-project 目录仍然存在"
