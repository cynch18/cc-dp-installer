...
除了源码脚本（`.ps1`），项目还提供了两个编译打包后的 **自解压 EXE 安装器**，存放在桌面，方便分发和离线使用。

### 1.1 文件清单

| 文件名 | 大小 | 类型 | 说明 |
|--------|------|------|------|
| `Install.exe` | 98.2 MB | 7-Zip SFX 自解压包 | 一键安装器（内嵌 install.ps1 + Node.js + Git + 依赖） |
| `Uninstall.exe` | 172 KB | 7-Zip SFX 自解压包 | 一键卸载器（内嵌 uninstall.ps1） |
| `Git-2.54.0-64-bit.exe` | 65.2 MB | Git for Windows 官方安装包 | Git 离线安装包（联网失败时的本地回退） |

### 1.2 技术细节

#### 打包方式

两个 EXE 均使用 **7-Zip SFX (Self-Extracting Archive)** 技术打包：

- **制作工具**：7-Zip 26.01（Copyright Igor Pavlov）
- **内部文件类型**：`7zS.sfx` — 控制台/GUI 自解压模块
- **格式**：PE32 可执行文件（GUI 模式），32位（兼容 64位 系统）
...
└────────────────────────────────────┘
         │
         ▼
  自动以管理员权限启动 install.ps1
  (或者需要用户手动以管理员运行)
```

#### Install.exe 内部构成（推测）

Install.exe 高达 98.2 MB，远大于纯脚本文件，说明它内嵌了离线安装资源：

| 内嵌资源 | 预估大小 | 用途 |
|----------|----------|------|
| `install.ps1` | ~38 KB | 主安装脚本 |
| Node.js `.msi` | ~25 MB | Node.js 18+ 离线安装包 |
| Git `.exe` | ~65 MB | Git for Windows 离线安装包（可能复用同目录的 Git-2.54.0-64-bit.exe） |
| 7-Zip SFX 开销 | ~8 MB | 自解压模块 + 压缩开销 |

> **注**：由于 Install.exe (98.2 MB) ≈ Git-2.54.0-64-bit.exe (65.2 MB) + Node.js MSI (~25 MB) + 脚本 (~38 KB) + 7-Zip SFX 开销，推测两个离线安装包都被打包进 Install.exe 中，使得安装器可以完全离线运行。
...
| 特性 | 源码 PS1 脚本 | EXE 自解压包 |
|------|:--:|:--:|
| 离线安装 | ❌ 需联网下载 | ✅ 内嵌安装包 |
| 可审计 | ✅ 纯文本 | ❌ 二进制不透明 |
| 文件大小 | 小 (~51 KB) | 大 (~100+ MB) |
| 分发便利 | 一般 | ✅ 单文件 |
| 杀软误报风险 | 低 | 较高（7-Zip SFX 常被误报） |
| 修改定制 | ✅ 直接编辑 | ❌ 需重新打包 |
