...
# 清理桌面快捷方式
Write-Host "  正在清理桌面 Claude Code 快捷方式..." -ForegroundColor Yellow
$shortcutPath = Join-Path $desktopPath "Claude Code.lnk"
if (Test-Path $shortcutPath) {
    try {
        Remove-Item -Path $shortcutPath -Force -ErrorAction SilentlyContinue
        Write-Host "  ✅ 已删除: Claude Code.lnk" -ForegroundColor Green
    } catch {
        Write-Host "  ⚠️ 无法删除快捷方式: $_" -ForegroundColor Yellow
    }
} else {
    Write-Host "  ℹ️ 未找到 Claude Code 桌面快捷方式" -ForegroundColor Gray
}
