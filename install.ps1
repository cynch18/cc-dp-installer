...
# 脚本级变量
$Script:EnvTempFile       = $null   # 环境变量临时文件路径（用于跨进程传递 + 安全清理）
$Script:OriginalExecPolicy = $null   # 保存原始执行策略，脚本退出前恢复

# 全局 trap — 必须在任何运行时语句之前定义
# 引用的函数在触发时才求值，此时所有函数已定义完毕
trap {
...
if (-not (Test-IsAdmin)) {
    ...
    # ═══════════════════════════════════════════════════════════════
    # 子进程已完成。环境变量已通过注册表持久化（HKCU\Environment），
    # 临时文件仅为兜底 IPC 通道（子进程清理后此路径不再生效）。
    # 使用子进程 PID 定位临时文件，防止固定文件名注入攻击
    # ═══════════════════════════════════════════════════════════════
    $childPid = $process.Id
    $envTempFile = Join-Path $env:TEMP "claude-code-env-${childPid}.ps1"
    if (Test-Path $envTempFile) {
        # 额外安全检查：验证文件大小合理（< 100KB），防止加载异常文件
        $tempFileSize = (Get-Item $envTempFile).Length
        if ($tempFileSize -gt 102400) {
            Write-Host "  ⚠️  临时文件大小异常 ($tempFileSize bytes)，跳过加载" -ForegroundColor Red
    ...
    # ── 从注册表解密 API Key（如果已配置）──
    # API Key 不经过临时文件 — 父进程直接从注册表读取加密 blob 并在内存中解密
    $encToken = [System.Environment]::GetEnvironmentVariable("ANTHROPIC_AUTH_TOKEN_ENC", "User")
    if ($encToken) {
        try {
            $bytes = [System.Convert]::FromBase64String($encToken)
            $decrypted = [System.Security.Cryptography.ProtectedData]::Unprotect(
                $bytes, $null, [System.Security.Cryptography.DataProtectionScope]::CurrentUser
            )
            $env:ANTHROPIC_AUTH_TOKEN = [System.Text.Encoding]::UTF8.GetString($decrypted)
        } catch {
            Write-Host "  ⚠️  API Key 解密失败，请使用桌面快捷方式启动" -ForegroundColor DarkYellow
        }
    }
    ...
    $dsEnc = [System.Environment]::GetEnvironmentVariable("ANTHROPIC_AUTH_TOKEN_ENC", "User")
    ...
    if ($dsEnc -and $dsUrl) { Write-Host "  ✅ DeepSeek : 已配置 (DPAPI 加密) → $dsUrl" -ForegroundColor Green } elseif ($dsUrl) { Write-Host "  ✅ DeepSeek : 已配置 → $dsUrl" -ForegroundColor Green } else { Write-Host "  ⚠️  DeepSeek : 未配置 (重新运行脚本并输入 Y)" -ForegroundColor Yellow }
    Write-Host ""
    Write-Host "  项目目录: $([Environment]::GetFolderPath("Desktop"))\my-project" -ForegroundColor Gray
    Write-Host "  桌面快捷方式: $([Environment]::GetFolderPath("Desktop"))\Claude Code.lnk" -ForegroundColor Gray
...
# 先保存原始执行策略，脚本退出前恢复
try {
    $Script:OriginalExecPolicy = (Get-ExecutionPolicy -Scope CurrentUser -ErrorAction SilentlyContinue)
} catch {
    $Script:OriginalExecPolicy = $null
}
try {
    Set-ExecutionPolicy RemoteSigned -Scope CurrentUser -Force
    Write-Host "  ExecutionPolicy: RemoteSigned (CurrentUser) — 脚本退出前自动恢复" -ForegroundColor Gray
} catch {
...
function Write-ErrorLog {
    ...
    # 先清理敏感临时文件 + 恢复执行策略，确保即使后续退出也不残留
    if ($Script:EnvTempFile -and (Test-Path $Script:EnvTempFile)) {
        Remove-Item $Script:EnvTempFile -Force -ErrorAction SilentlyContinue
    }
    if ($null -ne $Script:OriginalExecPolicy) {
        try {
            Set-ExecutionPolicy $Script:OriginalExecPolicy -Scope CurrentUser -Force -ErrorAction SilentlyContinue
        } catch { }
    }
    ...
    # 匹配 sk- 开头的密钥模式 和 ANTHROPIC_AUTH_TOKEN 相关的值
    $lastErr = $lastErr -replace '\bsk-[a-zA-Z0-9]{20,}\b', '[REDACTED-API-KEY]'
    $lastErr = $lastErr -replace "ANTHROPIC_AUTH_TOKEN\s*=\s*'[^']*'", "ANTHROPIC_AUTH_TOKEN='[REDACTED]'"
...
function Test-FileHash {
    ...
    .PARAMETER ExpectedHash
        预期的 SHA256 哈希值（不区分大小写）
    .PARAMETER Description
        文件描述（用于日志显示）
    .OUTPUTS
        返回 $true 表示哈希匹配，$false 表示不匹配或无法验证
    #>
    param(
        [string]$FilePath,
        [string]$ExpectedHash,
    ...
    if (-not $ExpectedHash -or -not (Test-Path $FilePath)) { return $false }
    ...
    try {
        ...
        Write-Host "  ⚠ SHA256 验证异常: $_" -ForegroundColor DarkYellow
        return $false
    }
}
...
function Install-Git {
    ...
    $tempDir = $env:TEMP
    $installerPath = Join-Path $tempDir "Git-Installer_$(New-Guid).exe"
    ...
    # 方案 A：尝试从 GitHub Release 下载 SHA256 校验文件
    try {
        $gitChecksumUrl = "https://github.com/git-for-windows/git/releases/download/$gitFullTag/$gitFileName.sha256sum"
        $gitChecksumContent = Invoke-WebRequest -Uri $gitChecksumUrl -UseBasicParsing -TimeoutSec 15 | Select-Object -ExpandProperty Content
        if ($gitChecksumContent) {
            $expectedHash = ($gitChecksumContent -split '\s+')[0].Trim().ToLowerInvariant()
            $gitVerified = Test-FileHash -FilePath $installerPath -ExpectedHash $expectedHash -Description "Git"
        }
    } catch {
        Write-Host "  ⚠️  SHA256 校验文件不可用，尝试 Authenticode 签名验证..." -ForegroundColor Yellow
    }

    # 方案 B：Authenticode 数字签名验证（Git for Windows 官方签名）
    if (-not $gitVerified) {
        try {
            ...
            if ($sig.Status -eq 'Valid') {
                ...
                Write-Host "    签名者: $($sig.SignerCertificate.Subject)" -ForegroundColor Gray
                $gitVerified = $true
            } elseif ($sig.Status -eq 'NotSigned') {
                Write-Host "  ⚠️  安装包无数字签名（可能非官方构建）" -ForegroundColor Yellow
            } else {
                Write-Host "  ⚠️  签名状态异常: $($sig.Status)" -ForegroundColor Yellow
            }
        } catch {
            Write-Host "  ⚠️  Authenticode 验证异常: $_" -ForegroundColor DarkYellow
        }
    }

    if (-not $gitVerified) {
        Write-Host "  ⚠️  无法验证 Git 安装包完整性！" -ForegroundColor Red
        Write-Host "     请手动核对: https://github.com/git-for-windows/git/releases/tag/$gitFullTag" -ForegroundColor Yellow
    }
    ...
    # 3. Git 静默安装参数
    $gitLogPath = Join-Path $tempDir "git-install.log"
    $installArgs = @(
        "/VERYSILENT",
        "/NORESTART",
        "/CLOSEAPPLICATIONS",
...
function New-ClaudeShortcut {
    ...
    try {
        ...
        # 核心变更：不再使用 cmd /k claude，而是通过启动器脚本
        # 启动器脚本在内存中解密 API Key 后启动 Claude Code
        if (Test-Path $launcherPath) {
            ...
            # 回退：如果没有启动器脚本（未配置 API Key），直接启动 claude
...
if ($hasNode18Plus) {
    ...
    $tempDir = $env:TEMP
    $installerPath = Join-Path $tempDir "node-$latestVersion-win-$arch-$(New-Guid).msi"
    ...
    # 0. SHA256 哈希校验（尝试从官方 SHASUMS256.txt 获取预期值）
    Write-Host "  🔐 正在验证 Node.js 安装包完整性..." -ForegroundColor Gray
    try {
        $shasumsUrl = "https://nodejs.org/dist/$latestVersion/SHASUMS256.txt"
        $shasumsContent = Invoke-WebRequest -Uri $shasumsUrl -UseBasicParsing -TimeoutSec 15 | Select-Object -ExpandProperty Content
        $nodeFileName = "node-$latestVersion-$arch.msi"
        # 用行尾锚点确保精确匹配文件名（防止 foo.msi 误匹配 foo.msi.sig）␊
        $expectedHash = ($shasumsContent -split "`n" | Where-Object { $_ -match ([regex]::Escape($nodeFileName) + '\s*$') }) -replace '\s+\*.*$', '' -replace '\s+.*$', '' -replace '\s', ''
        if ($expectedHash) {
            $hashOk = Test-FileHash -FilePath $installerPath -ExpectedHash $expectedHash -Description "Node.js"
            if (-not $hashOk) {
                Write-Host "  ⚠️  SHA256 哈希验证失败！安装包可能已损坏或被篡改。" -ForegroundColor Red
                Write-Host "  如需跳过验证继续安装，请手动删除此检查后重试。" -ForegroundColor Yellow
                Write-ErrorLog -Reason "Node.js SHA256 哈希验证失败 (预期: $expectedHash)"
                exit 1
            }
        } else {
            Write-Host "  ⚠️  无法从 SHASUMS256.txt 中解析哈希值，跳过验证" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "  ⚠️  无法获取官方 SHA256 校验和 (SHASUMS256.txt)，跳过验证" -ForegroundColor Yellow
    ...
    # ============================================================
    $msiLogPath = Join-Path $tempDir "node-install-$latestVersion.log"
    $maxMsiRetries = 3
    $msiSuccess = $false
...
if (-not $npmCmd) {
    ...
    if ($existingClaude) {
        ...
        if ($ccSuccess) {
            ...
            # 运行 npm audit 检查已知漏洞（带超时保护，超时 30s）
            Write-Host "  🔍 正在运行 npm audit 检查已知漏洞..." -ForegroundColor Gray
            try {
                $prevEAP = $ErrorActionPreference
                $ErrorActionPreference = "Continue"
                $auditJob = Start-Job -ScriptBlock {
                    param($npm, $pkg)
                    & $npm audit --json 2>&1 | Out-String
                } -ArgumentList $npmPath, $Package
                $auditResult = $auditJob | Wait-Job -Timeout 30 | Receive-Job
                ...
                if ($auditResult -match '"vulnerabilities"') {
                    ...
                    $totalVulns = $auditJson.metadata.vulnerabilities.total
                    if ($totalVulns -gt 0) {
                        Write-Host "  ⚠️  npm audit 发现 $totalVulns 个已知漏洞（详情: npm audit）" -ForegroundColor Yellow
                    } else {
                        Write-Host "  ✓ npm audit: 未发现已知漏洞" -ForegroundColor Green
                    }
                }
            } catch {
                Write-Host "  ℹ️  npm audit 不可用或超时，跳过" -ForegroundColor Gray
            }
...
if ($configureDeepseek -eq 'Y' -or $configureDeepseek -eq 'y') {
    ...
    if ($apiKey) {
        ...
        # 构建临时文件内容（仅非敏感变量！API Key 不进入临时文件）
        $envLines = @()

        # 写入加密的 API Key 到注册表
        [System.Environment]::SetEnvironmentVariable("ANTHROPIC_AUTH_TOKEN_ENC", $encryptedKey, "User")
        Write-Host "  ✓ `$env:ANTHROPIC_AUTH_TOKEN_ENC (DPAPI 加密)" -ForegroundColor Green
        ...
        # 写入临时文件（仅非敏感变量，供父进程 dot-source）
        # 使用当前进程 PID 作为文件名后缀，防止固定文件名注入攻击
        $Script:EnvTempFile = Join-Path $env:TEMP "claude-code-env-${PID}.ps1"
        try {
            $envLines -join "`n" | Out-File -FilePath $Script:EnvTempFile -Encoding UTF8 -Force
            Write-Host ""
            Write-Host "  ✓ 非敏感环境变量已写入注册表 + 临时文件" -ForegroundColor Green
            Write-Host "    (API Key 不在临时文件中 — 仅加密 blob 存于注册表)" -ForegroundColor Gray
        } catch {
...
# 该设置禁用 Claude Code 发出的请求中的归属标头。
# 设为 "0" 表示不发送归属信息，提高隐私性。
# 如需启用归属（帮助 Anthropic 改进产品），可手动删除此环境变量。
Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "  Claude Code 归属标头 (隐私)" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
[System.Environment]::SetEnvironmentVariable("CLAUDE_CODE_ATTRIBUTION_HEADER", "0", "User")
$env:CLAUDE_CODE_ATTRIBUTION_HEADER = "0"
Write-Host "  ✓ `$env:CLAUDE_CODE_ATTRIBUTION_HEADER = '0' (关闭归属，已持久化到用户注册表)" -ForegroundColor Green
Write-Host "    如需启用归属标头，请删除此环境变量" -ForegroundColor Gray
Write-Host ""
...
'@ | Out-File -FilePath $launcherPath -Encoding UTF8 -Force
    ...
    # 加固 ACL：仅当前用户和 SYSTEM 可修改启动器
    try {
        $acl = Get-Acl -Path $launcherPath
        ...
        Set-Acl -Path $launcherPath -AclObject $acl
        Write-Host "  🔒 启动器 ACL 已加固（仅当前用户可写）" -ForegroundColor Gray
    } catch {
        Write-Host "  ⚠️  ACL 加固失败（非关键）: $_" -ForegroundColor DarkYellow
    }
} catch {
...
try {
    ...
    Write-Host "  ⚠️  安装标记写入失败（非关键）: $_" -ForegroundColor DarkYellow
}
